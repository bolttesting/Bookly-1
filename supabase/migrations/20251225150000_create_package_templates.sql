-- Create package_templates table (business-created packages)
CREATE TABLE IF NOT EXISTS public.package_templates (
  ...
  duration_type TEXT NOT NULL CHECK (duration_type IN ('days', 'weeks', 'months', 'years')),
...
-- Create customer_packages table (when customer purchases a package)
...
-- Function to check if package is valid for booking

-- Free subscription invoices: store currency from plan, or business, then default (not always USD).
...
COMMENT ON FUNCTION public.record_free_platform_subscription_invoice IS
  'Creates a zero-amount Bookly subscription invoice in the plan currency (or business currency if plan has none).';
